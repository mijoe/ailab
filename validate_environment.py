...
def test_ollama_connection():
    ...
    try:
        ...
        for model in models[:5]:  # Show first 5
            model_name = model.model if hasattr(model, 'model') else model.get('model', 'unknown')
            param_size = model.details.parameter_size if hasattr(model, 'details') else model.get('details', {}).get('parameter_size', 'unknown')
